...
radon mi new_divar_scraper -s >>.logs/maintainability.txt
export maintainability=$(radon mi new_divar_scraper -s | grep -Po '(?<=\().*(?=\))' | perl -lane '$a+=$_ for(@F);$f+=scalar(@F);END{print $a/$f}' | xargs printf "%.1f%%\n")
